...
- 配分：
  $$
w_i \propto \frac{(\mathrm{Score}_i^+)^{\alpha}}{\sigma_i^{\beta}},\quad \alpha=1.0,\ \beta=0.5
$$
...
- 実効$\beta=\beta_{\text{long}}-\beta_{\mathrm{ETF}}\cdot\frac{Q_{\mathrm{ETF}}}{\mathrm{NAV}}$  
- 逸脱|Δβ|>0.1で自動リチューン
...
- 学習180日／運用60日、最小滞在5日  
- 判定：$R\ge \theta_H$ → 強気、$R\le \theta_L$ → 防御  
- $-2\sigma$で新規停止、$-3\sigma$で撤退
...
$$
\mathrm{GMI}=0.5\,Z(\text{S\&P先物})+0.3\,Z(\mathrm{VIX}^{-1})+0.2\,Z(\mathrm{USDJPY})
$$
...
$$
\mathbf{w}=\gamma\,\mathbf{w}^{(D)}+(1-\gamma)\,\mathbf{w}^{(\mathrm{IW})}
$$
