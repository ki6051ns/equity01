...
このモジュールは variant探索・実験用のスコアリング機能を提供します。
運用MVP（scripts/core/scoring_engine.py）とは分離されており、
coreを触らずにvariant探索が可能です。

注意: このファイルは core/scoring_engine.py から variant探索機能を抽出したものです。
"""
...
def compute_scores_all_variants(
    df: pd.DataFrame,
    base_col: str,
    *,
    ...
    """
    各 SCORING_VARIANTS ごとに score_<name> カラムを追加する（variant探索用）。
