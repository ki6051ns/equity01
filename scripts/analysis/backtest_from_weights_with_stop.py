...
"""
scripts/analysis/backtest_from_weights_with_stop.py

STOP付cross4 weightsからreturnsを計算するバックテスト（analysis内で完結、coreへ書き戻し禁止）

入力:
- data/processed/weights/cross4_target_weights_{strategy}_w{window}.parquet
- 価格データ

出力:
- data/processed/weights_bt/cross4_from_weights_{strategy}_w{window}.parquet（trade_date, port_ret_cc, cumulativeなど）
...
# プロジェクトルートをパスに追加
ROOT_DIR = Path(__file__).resolve().parents[2]  # scripts/analysis/backtest_from_weights_with_stop.py から2階層上
...
def load_inverse_returns() -> pd.Series:
    """
    インバースETFのリターンを読み込む（eval_stop_regimes.py準拠）
    
    Returns
    -------
    pd.Series
        インバースETFの日次リターン（日付をindexに）
    
    Raises
    ------
    FileNotFoundError
        インバースETFリターンファイルが見つからない場合（Plan Aでは必須）
    """
    # eval_stop_regimes.pyと同じロジックで読み込む
    # 複数のパスを試す（共通定数から）
    inverse_paths = [Path(p) for p in INVERSE_ETF_PRICE_PATHS]
    ...
    # 見つからない場合はエラー（Plan Aでは必須）
    raise FileNotFoundError(
        f"インバースETFリターンファイルが見つかりません。\n"
        f"試行したパス: {[str(p) for p in inverse_paths]}\n"
        f"Plan AではインバースETFリターンが必須です。"
    )
...
def calculate_returns_from_weights(
    ...
    strategy: str = None,
    inv_ret: pd.Series = None,
    stop_conditions: dict = None
) -> pd.DataFrame:
    """
    weightsからreturnsを計算する
    
    ✅ 定義: docs/return_definition.md を参照
    ...
    ----------
    df_weights : pd.DataFrame
        date, symbol, weight を含むDataFrame（STOPが既に適用されている前提）
    prices_df : pd.DataFrame, optional
        価格データ（指定しない場合はdata_loaderから読み込む）
    strategy : str, optional
        "stop0", "planA", "planB" のいずれか（現在は使用されていない、weightsファイルに既にSTOPが適用されているため）
    inv_ret : pd.Series, optional
        インバースETFのリターン（現在は使用されていない）
    stop_conditions : dict, optional
        STOP条件の辞書（現在は使用されていない）
    ...
    # リターンを計算（adj_closeを使用、docs/return_definition.md参照）
    # r_i[t] = adj_close_i[t] / adj_close_i[t-1] - 1
    ...
    stop_cond_for_planA = None
    inv_ret_aligned = None
    if strategy == "planA" and inv_ret is not None and len(inv_ret) > 0 and stop_conditions is not None:
        # 60日または120日のSTOP条件を使用（windowはstop_conditionsのキーから取得）
        # 簡易的に最初のwindowを使用
        window_key = list(stop_conditions.keys())[0] if stop_conditions else None
        if window_key:
            stop_cond_for_planA = stop_conditions[window_key]
            # inverse ETFリターンを日付に合わせる
            inv_ret_aligned = inv_ret.reindex(stop_cond_for_planA.index, fill_value=0.0)
...
def print_monthly_yearly_performance(df_alpha: pd.DataFrame, name: str):
    ...
    monthly, yearly = compute_monthly_perf(
        ...
        out_prefix=str(DATA_DIR / "weights_bt")
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="STOP付cross4 weightsからreturnsを計算するバックテスト"
    )
    parser.add_argument(
        "--strategy",
        type=str,
        required=True,
        choices=["stop0", "planA", "planB"],
        help="戦略（stop0, planA, planB）"
    )
    parser.add_argument(
        "--window",
        type=int,
    ...
    print("=" * 80)
    print(f"=== STOP付cross4 Weights→Returns計算 ({args.strategy.upper()}, window={args.window}) ===")
    print("=" * 80)
    
    # weightsを読み込む
    weights_path = WEIGHTS_DIR / f"cross4_target_weights_{args.strategy}_w{args.window}.parquet"
    print(f"\n[STEP 1] weightsを読み込み中: {weights_path}")
    try:
        df_weights = load_weights(weights_path)
        print(f"  [OK] {len(df_weights)} rows")
    except FileNotFoundError as e:
        print(f"  [ERROR] {e}")
        return 1
    
    # インバースETFリターンとSTOP条件を読み込む（Plan A用）
    inv_ret = None
    stop_conditions = None
    if args.strategy == "planA":
        print(f"\n[STEP 1.5] インバースETFリターンを読み込み中（Plan A用）...")
        inv_ret = load_inverse_returns()
        if len(inv_ret) > 0:
            print(f"  [OK] {len(inv_ret)} 日分")
        else:
            print(f"  ⚠ インバースETFリターンが見つかりません。0リターンで埋めます。")
        
        # STOP条件を計算（Plan Aで使用）
        print(f"\n[STEP 1.6] STOP条件を計算中（Plan A用）...")
        try:
            # cross4 returnsを読み込む
            from scripts.analysis.build_cross4_target_weights_with_stop import load_cross4_returns_for_stop, compute_stop_conditions
            ...
            port_ret = df_returns_for_stop.set_index("trade_date")["port_ret_cc"]
            tpx_ret = df_returns_for_stop.set_index("trade_date")["tpx_ret_cc"]
            stop_conditions = compute_stop_conditions(port_ret, tpx_ret, windows=[args.window])
            print(f"  [OK] STOP条件計算完了（window={args.window}）")
        except Exception as e:
            print(f"  [ERROR] STOP条件計算エラー: {e}")
            import traceback
            traceback.print_exc()
            print(f"\n[ERROR] Plan AではSTOP条件計算が必須です。計算に失敗したため処理を中断します。")
            return 1
    
    # returnsを計算
    print(f"\n[STEP 2] returnsを計算中...")
    df_result = calculate_returns_from_weights(
        df_weights,
    ...
    if df_result.empty:
        ...
        print(f"  [OK] {len(df_result)} 日分")
    
    # 保存
    output_path = WEIGHTS_BT_DIR / f"cross4_from_weights_{args.strategy}_w{args.window}.parquet"
    ...
    strategy_name = f"{strategy_name_map.get(args.strategy, args.strategy.upper())} {args.window}d"
    ...
    print("\n" + "=" * 80)
    print("完了: STOP付cross4 Weights→Returns計算完了")
    print("=" * 80)
