"""
DEPRECATED: このファイルは使用されていません。

このファイルは core/scoring_engine.py から移動された variant探索用の旧実装です。

現在の運用MVPは scripts/core/scoring_engine.py を使用してください。
variant探索が必要な場合は scripts/analysis/scoring_variants.py を使用してください。

移動日: 2025-01-XX
移動理由: coreを1か所・1実装に統一するため
"""

# 以降は core/scoring_engine.py の内容をそのまま保持
